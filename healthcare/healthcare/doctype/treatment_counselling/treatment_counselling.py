...
def set_treatment_plan_template_items(doc):
	if doc.treatment_plan_template and not doc.treatment_plan_template_items:
		...
		for item in item_list:
			...
			if item_details:
				item_price = [element for tupl in item_details for element in tupl][1]
		...
		if doc.admission_service_unit_type:
			...
			if su_item and su_rate:
				doc.append(
					"treatment_plan_template_items", {"type":"Item", "template": su_item, "qty": doc.expected_length_of_stay, "amount": su_rate},
				)
